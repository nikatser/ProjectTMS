...
class RegisterVC: UIViewController {
    ...
    var appNavigationController = UINavigationController(rootViewController: AppTabBarC())
    ...
    let textLabel: UILabel = {
        var label = UILabel()
        label.translatesAutoresizingMaskIntoConstraints = false
        label.text = "You password must be 8 or more characters long and containa mix upper&lower case letters, number and symbol"
        label.font = UIFont(name: Constant.shared.mainNameFont, size: 13)
        label.numberOfLines = 0
        label.textColor = #colorLiteral(red: 0.4274509549, green: 0.4274509549, blue: 0.4274509549, alpha: 1)
    ...
    private func setupViews() {
        ...
        scrollView.addSubview(joinUsButton)
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            nameLabel.topAnchor.constraint(equalTo: mainLabel.topAnchor, constant: 77),
            nameLabel.leftAnchor.constraint(equalTo: scrollView.leftAnchor, constant: 24),
            
            backButton.topAnchor.constraint(equalTo: scrollView.topAnchor, constant: 77),
            backButton.leftAnchor.constraint(equalTo: scrollView.leftAnchor, constant: 24),
            
            nameLabel.topAnchor.constraint(equalTo: nameLabel.bottomAnchor, constant: 60),
            nameLabel.leftAnchor.constraint(equalTo: scrollView.leftAnchor, constant: 24),
            
            nameTextField.topAnchor.constraint(equalTo: nameLabel.bottomAnchor, constant: 10),
            nameTextField.leftAnchor.constraint(equalTo: scrollView.leftAnchor, constant: 24),
            nameTextField.rightAnchor.constraint(equalTo: scrollView.rightAnchor, constant: -24),
            ...
            textLabel.topAnchor.constraint(equalTo: passwordTextField.bottomAnchor, constant: 10),
            textLabel.leftAnchor.constraint(equalTo: scrollView.leftAnchor, constant: 24),
            textLabel.rightAnchor.constraint(equalTo: scrollView.rightAnchor, constant: -24),
            
            joinUsButton.topAnchor.constraint(equalTo: textLabel.bottomAnchor, constant: 50),
            joinUsButton.leftAnchor.constraint(equalTo: scrollView.leftAnchor, constant: 40),
            joinUsButton.rightAnchor.constraint(equalTo: scrollView.rightAnchor, constant: -40),
        ])
    }
    
    @objc
    func app(){
        if CheakValid.shared.validRegister(name: nameTextField.text ?? "", email: emailTextField.text ?? "", password: passwordTextField.text ?? "") {
            Service.shared.createNewUser(LoginField(email: emailTextField.text!, password: passwordTextField.text!),name: nameTextField.text!) {code in
                switch code.code{
                case 0:
                    print("ERROR")
                case 1:
